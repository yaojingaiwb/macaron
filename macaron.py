...
class Twitter:
    def __init__(self, auth_token):
        ...
        self.Twitter = AsyncSession(
            headers=defaulf_headers,
            cookies=defaulf_cookies,
            timeout=120,
...
class Macaron:
    def __init__(self, private_key: str, auth_token: str, proxy: str):
        # session = "".join(
        #     random.choice(string.digits + string.ascii_letters) for _ in range(10)
        # )
        # nstproxy = f"http://{nstChannelID}-residential-country_ANY-r_5m-s_{session}:{nstPassword}@gw-us.nstproxy.io:24125"
        # print(nstproxy)
        print(private_key, auth_token, proxy)
        self.w3 = AsyncWeb3(
            AsyncWeb3.AsyncHTTPProvider(
                "https://arbitrum.blockpi.network/v1/rpc/public"
            )
        )
        self.client = AsyncSession(proxy=proxy, timeout=120, impersonate="chrome120")
    ...
    async def login(self):
        try:
            ...
            res = await self.client.post(
                "https://api.macaron.xyz/auth/login", json=json_data
            )
    ...
    async def getAuthUrl(self):
        try:
            ...
            res = await self.client.get(
                "https://info-api.macaron.xyz/twitter/auth_url", params=params
            )
            print(res.json())
    ...
    async def bindTwitter(self, client_id, state, code_challenge):
        try:
            ...
            res = await self.client.get(
                "https://info-api.macaron.xyz/twitter/callback",
                params=params,
    ...
    async def verify(self):
        try:
            res = await self.client.get(
                f"https://info-api.macaron.xyz/twitter/verify?wallet_address={self.account.address}&type=follow"
            )
            if res.status_code == 200 and res.json()["statusCode"] == 200:
    ...
    async def completed(self, taskID):
        try:
            ...
            res = await self.client.post(
                f"https://api.macaron.xyz/points/task/completed", json=json_data
            )
    ...
    async def claim(self, taskID):
        try:
            json_data = {"task_id": taskID}
            res = await self.client.post(
                f"https://api.macaron.xyz/points/task/claimed", json=json_data
            )
...
async def do(semaphore, private_key, auth_token, proxy):
    async with semaphore:
        for _ in range(3):
            if await Macaron(private_key, auth_token, proxy).login():
                break


async def main(filePath, proxyPath, tread):
    semaphore = asyncio.Semaphore(int(tread))
    task = []
    with open(filePath, "r") as a, open(proxyPath, "r") as p:
        accountsz = a.readlines()
        proxysz = p.readlines()

    for account, proxy in zip(accountsz, proxysz):
        private_key, auth_token = account.strip().split("----")
        proxyx = proxy.strip().split(":")
        proxyy = f"http://{proxyx[2]}:{proxyx[3]}@{proxyx[0]}:{proxyx[1]}"
        task.append(do(semaphore, private_key, auth_token, proxyy))
    await asyncio.gather(*task)


if __name__ == "__main__":
    _filePath = r"自己的目标目录 或者 运行目录(只适用win) linux得自己改 我懒得改了"
    _proxyPath = r"自己的目标目录 或者 运行目录(只适用win) linux得自己改 我懒得改了"
